...
from clip_textrefiner.model import convert_weights
...
def parse_args():
    ...
    parser.add_argument("--output-dir", type=str, default="./outputs_textrefiner")
...
if __name__ == "__main__":
    ...
    optimizer_text_prompt = torch.optim.AdamW(trainable_list.parameters(), lr=1e-3, weight_decay=0.8)
    ...
    for epoch in range(args.epochs):
        step_lr_schedule(optimizer_text_prompt, epoch, init_lr=1e-3, min_lr=5e-4, decay_rate=0.01)
        ...
        for (imgs, labels, _) in pbar:
            ...
            with autocast():
                text_features, image_features, logit_scale, image_fine, top_image_fine = model(imgs)
                fine_text_features, loss3 = memory(text_features, image_fine)
                output = logit_scale * image_features @ fine_text_features.t()
                top_local_logit = logit_scale * top_image_fine @ text_features.t()
                loss1 = F.cross_entropy(output, labels)
                loss2 = F.cross_entropy(top_local_logit, labels.repeat_interleave(dim=0, repeats=7))
                loss = loss1 + loss2 / args.balance + loss3 * args.distill
        ...
        if top1[0] >= best_top1:
            best_top1 = top1[0]
            save_obj = {
                'textrefiner_prompt_learner': model.prompt_learner.state_dict(),
                "memory_item": memory.state_dict(),
                'epoch': epoch,
                'best_top1': best_top1,
                'top5': top5[0],
                'seed': args.seed
