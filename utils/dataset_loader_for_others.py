...
def get_data_handler(dataset, pattern, input_size):
    if dataset == 'CIFAR100':
        ...
        if pattern == "train":
            datahandler = CIFAR100_handler_train(train_data, train_label, transform=get_transform(input_size))
        elif pattern == "val":
...
def divide_labeled_or_not_gened_clip(dataset, input_size):
    data_handler = get_data_handler_gened_clip(dataset, pattern='train', input_size=input_size)
    yt_tensor = torch.tensor(data_handler.YT)
    is_labeled = yt_tensor == 1
    labeled_indices = is_labeled.nonzero().squeeze().tolist()
    unlabeled_indices = (~is_labeled).nonzero().squeeze().tolist()
    return Subset(data_handler, labeled_indices), Subset(data_handler, unlabeled_indices)
